...
window.SmartModelSelector = (function() {
    ...
    function isEdgeProxy() {
        return String(config.apiUrl || '').startsWith('/');
    }
    ...
    return {
        ...
        callAI: async function(message, options = {}) {
            ...
            const useEdgeProxy = isEdgeProxy();
            ...
            for (let retry = 0; retry < config.maxRetries; retry++) {
                ...
                try {
                    ...
                    const edgeBody = {
                        message,
                        messages: requestBody.messages.filter(item => item.role !== 'system'),
                        mode: options.strategy || config.defaultStrategy || 'balanced',
                    ...
                    const response = await fetch(config.apiUrl, {
                        ...
                        headers: {
                            'Content-Type': 'application/json',
                            ...(useEdgeProxy ? {} : { 'Authorization': `Bearer ${config.apiKey}` })
                        },
                        body: JSON.stringify(useEdgeProxy ? edgeBody : requestBody),
                    ...
                    if (useEdgeProxy) {
                        if (!result.ok || !result.content) {
                            throw new Error(result.error || 'AI代理返回为空');
                        }
