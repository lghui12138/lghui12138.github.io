...
window.QuestionBankEngine = (function() {
    ...
    function isDisplayableQuestion(q) {
        ...
        if (q.type === '说明项') return false;
        if (Array.isArray(q.tags) && q.tags.includes('分页占位')) return false;
        return true;
    }
    ...
    async function loadFromGitHubRaw(bankName) {
        ...
        const data = await response.json();
        return (Array.isArray(data) ? data : (data.questions || [])).filter(isDisplayableQuestion);
    }
    ...
    async function loadFromGitHubAPI(bankName) {
        ...
        return (Array.isArray(content) ? content : (content.questions || [])).filter(isDisplayableQuestion);
    }
