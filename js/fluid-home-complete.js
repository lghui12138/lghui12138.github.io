...
(function(){
	...
	function normalizeHomeServerStats(payload){
	  ...
	  return {
	    ...
	    accuracy:Number(stats.accuracy||0),
		    ...
		    durablePrimary:payload.durablePrimary===true,
		    fullProductionReady:payload.fullProductionReady===true,
		    degradedKvFallback:payload.degradedKvFallback===true,
		    strictCumulativeServer:payload.strictCumulativeServer===true,
		    serverUpgradeInvariant:String(payload.serverUpgradeInvariant||''),
		    writeDurabilityGate:payload.writeDurabilityGate&&typeof payload.writeDurabilityGate==='object'?payload.writeDurabilityGate:{},
	...
	function applyHomeServerStats(snapshot,opts){
	  ...
	  const statsBox=document.getElementById('stats');
		  if(statsBox){
		    ...
		    statsBox.dataset.strictCumulativeServer=snapshot.strictCumulativeServer?'true':'false';
		    statsBox.dataset.storeMode=snapshot.storeMode||'';
		    statsBox.setAttribute('aria-label','我的服务端累计统计：登录、刷新和服务器升级不会从本机重新计算');
		  }
		  renderHomeServerHealthStatus();
		  return true;
		}
	...
	function setHomeStatsBlocked(){
	  ...
	  const statsBox=document.getElementById('stats');
		...
		function renderHomeServerHealthStatus(){
		  const el=document.getElementById('serverHealthLine');
		  if(!el)return;
		  const h=HOME_SERVER_HEALTH_STATE.snapshot;
		  const p=HOME_SERVER_PROGRESS_STATE.snapshot;
		  if(h&&h.claims){
		    const lp=h.learningProgress||{};
		    const pv=h.privateVideo||{};
		    const pe=h.publicEntry||{};
		    el.dataset.serverHealth='round402';
		    el.dataset.noMutationRead=h.noMutationRead?'true':'false';
		    el.dataset.cumulativeSourceOfTruth=h.cumulativeSourceOfTruth||'server-progress-snapshot';
		    el.dataset.progressStoreMode=lp.storeMode||'';
		    el.dataset.strictProgressWritesReady=h.claims.strictProgressWritesReady?'true':'false';
		    el.dataset.privateVideoStorageReady=h.claims.privateVideoStorageReady?'true':'false';
		    el.dataset.httpEntryReady=h.claims.httpEntryReady?'true':'false';
		    const progressLabel=h.claims.strictProgressWritesReady?'D1/R2 strict':(h.claims.serverKvCumulativeFallback?'KV 服务端累计，D1/R2 strict 未绑定':'进度存储未就绪');
		    const videoLabel=h.claims.privateVideoStorageReady?'私有视频 R2 就绪':'私有视频 R2 未就绪';
		    const publicLabel=h.claims.httpEntryReady?'HTTP 入口已证':'HTTP 入口需外部复验';
		    el.textContent='服务器状态：'+progressLabel+'；'+videoLabel+'；'+publicLabel+'。累计显示只认 /api/stats noMutationRead 快照，本机记录和最近事件窗不入累计。';
		    el.title=[lp.boundary,pv.boundary,pe.lastKnownBlocker].filter(Boolean).join(' ');
		    return;
		  }
		  if(p){
		    el.dataset.serverHealth='progress-only';
		    el.dataset.noMutationRead=p.noMutationRead?'true':'false';
		    el.dataset.cumulativeSourceOfTruth=p.cumulativeSourceOfTruth||'server-progress-snapshot';
		    el.textContent='服务器状态：已读取服务端累计快照；等待生产绑定健康检查。';
		    return;
		  }
		  el.dataset.serverHealth='pending';
		  el.textContent=HOME_SERVER_PROGRESS_STATE.status==='blocked'?'服务器状态：累计快照不可用，页面不使用本机数据冒充累计。':'服务器状态：正在读取 noMutationRead 服务端累计快照。';
		}
		...
		async function loadHomeServerHealth(){
		  renderHomeServerHealthStatus();
		  try{
		    const response=await fetch('/_edge-server-health',{method:'GET',credentials:'same-origin',cache:'no-store',headers:{Accept:'application/json'}});
		    if(!response.ok){renderHomeServerHealthStatus();return false}
		    const payload=await response.json().catch(()=>null);
		    if(!payload||payload.noMutationRead!==true){renderHomeServerHealthStatus();return false}
		    HOME_SERVER_HEALTH_STATE.status='ready';
		    HOME_SERVER_HEALTH_STATE.snapshot=payload;
		    HOME_SERVER_HEALTH_STATE.syncedAt=payload.generatedAt||new Date().toISOString();
		    renderHomeServerHealthStatus();
		    return true;
		  }catch(_){
		    HOME_SERVER_HEALTH_STATE.status='blocked';
		    HOME_SERVER_HEALTH_STATE.snapshot=null;
		    renderHomeServerHealthStatus();
		    return false;
		  }
		}
...
async function loadUpd(){
  ...
  const fb=[
    ...
    {time:'2026-05-06 20:05',title:'2010 第五题与知识点全库公式源同步修复',summary:'恢复 2010 年第五题真实三维速度场题；把 P78、P82、P114 的边界层与源汇叠加公式从复校源同步回网站，知识点全库 202 页 MathJax 扫描错误和拆行残片均为 0。',links:[{label:'2010 真题',url:'/modules/real-exams-dynamic.html?edge_refresh=round588-question-bank-home-conductor-20260630&year=2010'},{label:'知识点全集',url:'/modules/knowledge-detail.html'},{label:'更新记录',url:'/site-updates.json'}]},
    {time:'2026-05-06 18:11',title:'2003 年真题按扫描题面核验为 7 道整题',summary:'把 2003 年旧候选跳号碎片收口为扫描对应的 7 道整题，修正等压线题、压差计流量公式和复势 F(z)=-(m/2π)ln(z-1/z) 的源汇净流量口径，并重建知识点强相关真题跳转。',links:[{label:'2003 真题',url:'/modules/real-exams-dynamic.html?edge_refresh=round588-question-bank-home-conductor-20260630&year=2003'},{label:'知识点全集',url:'/modules/knowledge-detail.html'},{label:'更新记录',url:'/site-updates.json'}]},
    {time:'2026-05-06 16:37',title:'2002 年真题按扫描题面核验为 8 道整题',summary:'把 2002 年旧候选 14 条碎片收口为扫描对应的 8 道整题，修正自由涡、旋涡拉伸、速度势压力、两层 Poiseuille 与边界层/Bernoulli 综合题，并清掉知识点全库里的旧题号跳转。',links:[{label:'2002 真题',url:'/modules/real-exams-dynamic.html?edge_refresh=round588-question-bank-home-conductor-20260630&year=2002'},{label:'知识点全集',url:'/modules/knowledge-detail.html'},{label:'更新记录',url:'/site-updates.json'}]},
    {time:'2026-05-06 15:36',title:'2001 年真题按扫描题面核验为 8 道整题',summary:'按 mac_2T 当前真题 Word 与扫描源索引重核 2001 年分年包，修正第 3 题“流速与半径成反比”的自由涡答案，并让旧候选误读退出用户侧口径。',links:[{label:'2001 真题',url:'/modules/real-exams-dynamic.html?edge_refresh=round588-question-bank-home-conductor-20260630&year=2001'},{label:'知识点全集',url:'/modules/knowledge-detail.html'},{label:'更新记录',url:'/site-updates.json'}]},
    {time:'2026-05-06 15:00',title:'2004 年真题补为 6 道已核验整题',summary:'按 mac_2T 当前真题 Word 与扫描源索引，把 2004 年填空、名词解释、基本方程、小孔自由射流、流函数涡量方程和运动边界题整理成 6 道整题，并重建知识点强相关真题跳转。',links:[{label:'2004 真题',url:'/modules/real-exams-dynamic.html?edge_refresh=round588-question-bank-home-conductor-20260630&year=2004'},{label:'知识点全集',url:'/modules/knowledge-detail.html'},{label:'更新记录',url:'/site-updates.json'}]},
    {time:'2026-05-06 14:25',title:'2006 年真题补为 7 道已核验整题',summary:'按 mac_2T 当前真题 Word 与扫描源索引，把 2006 年选择题、点源、旋转圆柱耗散、肥皂泡、运动边界、点涡镜像和 Couette 流整理成 7 道整题，旧碎片退出知识点跳转。',links:[{label:'2006 真题',url:'/modules/real-exams-dynamic.html?edge_refresh=round588-question-bank-home-conductor-20260630&year=2006'},{label:'知识点全集',url:'/modules/knowledge-detail.html'},{label:'更新记录',url:'/site-updates.json'}]},
    {time:'2026-05-06 13:55',title:'2005 年真题补为 8 道已核验整题',summary:'按 mac_2T 当前真题 Word 同步 2005 年源题面，把收缩管、二维速度场、明渠连续方程、小孔出流、点源势流、两层 Couette 流和边界层/Reynolds 应力题补成已核验整题，并重建知识点强相关真题跳转。',links:[{label:'2005 真题',url:'/modules/real-exams-dynamic.html?edge_refresh=round588-question-bank-home-conductor-20260630&year=2005'},{label:'知识点全集',url:'/modules/knowledge-detail.html'},{label:'更新记录',url:'/site-updates.json'}]},
    {time:'2026-05-06 12:35',title:'2007 年真题补为 8 道已核验整题',summary:'把 2007 年扫描题面从旧碎片整理成 8 道整题，修正选择题、收缩管、非定常流线轨迹、文托利、Couette 和点源镜像题，并重建知识点强相关真题跳转。',links:[{label:'2007 真题',url:'/modules/real-exams-dynamic.html?edge_refresh=round588-question-bank-home-conductor-20260630&year=2007'},{label:'知识点全集',url:'/modules/knowledge-detail.html'},{label:'更新记录',url:'/site-updates.json'}]},
    {time:'2026-05-06 11:45',title:'2008 年真题补为 4 道已核验整题',summary:'把 2008 年扫描题面 4 个大题与笔记第 6、41、63、69-70、145、147 页及基础知识页逐题对齐，修正连续方程、扫描速度场和容器图示题错接，并重建知识点强相关真题跳转。',links:[{label:'2008 真题',url:'/modules/real-exams-dynamic.html?edge_refresh=round588-question-bank-home-conductor-20260630&year=2008'},{label:'知识点全集',url:'/modules/knowledge-detail.html'},{label:'更新记录',url:'/site-updates.json'}]},
    {time:'2026-05-06 09:30',title:'2009 年真题补为 8 道已核验整题',summary:'把 2009 年扫描题面 8 个大题与笔记第 4、8-9、57、69-70 页及基础知识页逐题对齐，补回 Helmholtz 分解题，修正点汇复势和等温理想气体圆锥管速度关系，并重建知识点强相关真题跳转。',links:[{label:'2009 真题',url:'/modules/real-exams-dynamic.html?edge_refresh=round588-question-bank-home-conductor-20260630&year=2009'},{label:'知识点全集',url:'/modules/knowledge-detail.html'},{label:'更新记录',url:'/site-updates.json'}]},
    {time:'2026-05-06 09:09',title:'2010 年真题补为 6 道已核验整题',summary:'把 2010 年扫描题面 6 个大题与笔记第 96-98 页及基础知识页逐题对齐，修正旧包里的非定常轨迹、单摆量纲和三维涡量题 OCR 残留，并重建知识点强相关真题跳转。',links:[{label:'2010 真题',url:'/modules/real-exams-dynamic.html?edge_refresh=round588-question-bank-home-conductor-20260630&year=2010'},{label:'知识点全集',url:'/modules/knowledge-detail.html'},{label:'更新记录',url:'/site-updates.json'}]},
    {time:'2026-05-06 08:52',title:'2012 年真题补为 7 道已核验整题',summary:'把 2012 年扫描题面 7 个大题与笔记第 91-93 页、Couette 基础页逐题对齐，修正平面流 OCR 残缺和运动边界平方项口径，并重建知识点强相关真题跳转。',links:[{label:'2012 真题',url:'/modules/real-exams-dynamic.html?edge_refresh=round588-question-bank-home-conductor-20260630&year=2012'},{label:'知识点全集',url:'/modules/knowledge-detail.html'},{label:'更新记录',url:'/site-updates.json'}]},
    {time:'2026-05-06 08:35',title:'2013 年真题补为 8 道已核验整题',summary:'把 2013 年扫描题面 8 个大题与笔记第 94-95 页及基础知识页逐题对齐，修正等压线题 OCR 差异，并重建知识点强相关真题跳转。',links:[{label:'2013 真题',url:'/modules/real-exams-dynamic.html?edge_refresh=round588-question-bank-home-conductor-20260630&year=2013'},{label:'知识点全集',url:'/modules/knowledge-detail.html'},{label:'更新记录',url:'/site-updates.json'}]},
    {time:'2026-05-06 08:10',title:'2014 年真题补为 6 道已核验整题',summary:'把 2014 年扫描题面 6 个大题与笔记第 96-99 页答案线索对齐，剔除附录误入题库、点涡 OCR 和同轴管层流旧错解，并重建知识点强相关真题跳转。',links:[{label:'2014 真题',url:'/modules/real-exams-dynamic.html?edge_refresh=round588-question-bank-home-conductor-20260630&year=2014'},{label:'知识点全集',url:'/modules/knowledge-detail.html'},{label:'更新记录',url:'/site-updates.json'}]},
    {time:'2026-05-06 07:40',title:'2015 年真题补为 5 道已核验整题',summary:'把 2015 年扫描题面 5 个大题与笔记第 100-103 页答案线索对齐，清掉旧候选题号和旧坐标残留，并重建知识点强相关真题跳转。',links:[{label:'2015 真题',url:'/modules/real-exams-dynamic.html?edge_refresh=round588-question-bank-home-conductor-20260630&year=2015'},{label:'知识点全集',url:'/modules/knowledge-detail.html'},{label:'更新记录',url:'/site-updates.json'}]},
    {time:'2026-05-06 07:10',title:'2016 年真题补为 9 道已核验整题',summary:'把 2016 年扫描题面 9 个大题与笔记第 104-107 页答案线索对齐，旧碎片题退场，并为 Froude 相似、流动分离和 Stokes 第一问题补准公式支撑。',links:[{label:'2016 真题',url:'/modules/real-exams-dynamic.html?edge_refresh=round588-question-bank-home-conductor-20260630&year=2016'},{label:'知识点全集',url:'/modules/knowledge-detail.html'},{label:'更新记录',url:'/site-updates.json'}]},
    {time:'2026-05-06 06:28',title:'2018 年真题补为 6 道已核验整题',summary:'把 2018 年扫描题面 6 个大题与笔记第 108-109 页答案线索对齐，并补强知识点全库 psi、vec、boldsymbol 等公式渲染。',links:[{label:'历年真题',url:'/modules/real-exams-dynamic.html?edge_refresh=round588-question-bank-home-conductor-20260630&year=2018'},{label:'知识点全集',url:'/modules/knowledge-detail.html'},{label:'更新记录',url:'/site-updates.json'}]},
    {time:'2026-05-06 06:12',title:'2019 年真题补为 5 道已核验整题',summary:'把 2019 年扫描题面 5 个大题与笔记第 110-111 页答案对齐，知识点跳转和分年真题包同步重建。',links:[{label:'历年真题',url:'/modules/real-exams-dynamic.html?edge_refresh=round588-question-bank-home-conductor-20260630&year=2019'},{label:'知识点全集',url:'/modules/knowledge-detail.html'},{label:'更新记录',url:'/site-updates.json'}]},
...
const SEARCH_IDX=[
// ==== 主入口（4 大学习中心）====
{t:'入口',n:'公式回查表',u:'/index-complete.html#formula-checklist',d:'首页第一屏复习路线：选出公式后核适用条件、边界条件、单位方向和常见错因，再回看对应例题和自制动画。',k:'公式回查表 适用条件 边界条件 单位方向 常见错因 自制动画 知识整理入口 连续方程 伯努利方程 动量方程 量纲分析 黏性近似'},
{t:'入口',n:'条件回查交互工作台',u:'/index-complete.html#formula-checklist',d:'首页学习工作台的条件回查标签：搜索公式，逐条核适用条件、边界条件、单位方向、常见错因和补救训练。',k:'条件回查 交互工作台 公式条件 适用条件 边界条件 单位方向 常见错因 补救训练'},
...
{t:'入口',n:'181103 资料题库与练习入口',u:'/modules/question-bank.html?focus=181103-material-extracted&answer_status=current&edge_refresh=round588-question-bank-home-conductor-20260630#questionBanksList',d:'首页搜索 181103 可直接进入 522 张来源 HTML 卡和网页答案块；400 道默认练习题可直接看参考答案，0 道待人工源页复核，122 条源文线索只展示；同时保留 38/38 HTML 资料总表和 68 个真题复核任务；用户搜“181103去哪了、参考答案状态、181103里面还有别的题目、资料题库”也直达这里。',k:'181103 181103资料 181103资料题库 181103去哪了 181103那些资源去哪了 181103资源看不见 181103资料去哪了 181103里面的题目 181103里面还有别的题目 181103资料内题 181103题库 资料题库 参考答案状态 答案状态 可直接参考答案 待人工源页复核 400可参考 400+0 122源文线索 522来源卡 522资料内题 522个题 38/38 HTML 68真题复核 题库 练习 搜索入口'},
...
function renderChapterPracticeLinks(nodes){
  return '<div class="path-links" aria-label="六章全部真题练习入口与独立模拟章节题入口">'+nodes.map(n=>'<a href="'+esc(n.p||'/modules/real-exams-dynamic.html?edge_refresh=round588-question-bank-home-conductor-20260630&from=current-student-path')+'">'+esc(n.n)+'做全部真题练习</a><a href="'+esc(n.m||'/modules/simulated-exams-dynamic.html?from=student-path')+'">'+esc(n.n)+'模拟题（非真题）</a>').join('')+'<span class="path-note">模拟章节题来自教材主题启发，独立题包，不混入正式真题。</span></div>';
}
function renderPathNext(nodeStatus,doneCount,total){
  const box=$('#pathNext');if(!box)return;
  if(doneCount>=total){
    box.innerHTML='<strong>六章主线已完成。</strong> 下一步进入 <a href="/modules/real-exams-dynamic.html?edge_refresh=round588-question-bank-home-conductor-20260630&from=current-student-path-complete">历年真题</a> 做一套限时回顾。'+renderChapterPracticeLinks(nodeStatus);
    return;
  }
  const current=nodeStatus.find(n=>n.cur)||nodeStatus.find(n=>!n.done)||nodeStatus[0];
  const idx=nodeStatus.indexOf(current)+1;
  box.innerHTML='<strong>继续学习 '+idx+'/'+total+'：'+esc(current.n)+'</strong> · '+esc(current.desc)+'。<a href="'+esc(current.u||'/modules/knowledge-detail.html')+'">学本章</a> · <a href="'+esc(current.p||'/modules/real-exams-dynamic.html?edge_refresh=round588-question-bank-home-conductor-20260630&from=student-path')+'">做全部真题练习</a>，做完后回这里进入下一章。'+renderChapterPracticeLinks(nodeStatus);
}
function renderPath(){
    ...
    const cards=[
      {
        ...
        href:path.doneCount>=path.total?'/modules/real-exams-dynamic.html?edge_refresh=round588-question-bank-home-conductor-20260630&from=round273-radar-complete':(next.p||'/modules/real-exams-dynamic.html?edge_refresh=round588-question-bank-home-conductor-20260630&from=round273-radar'),
...
const MOD_NAMES={'fluid-statics':'流体静力学','kinematics':'运动学','dynamics':'动力学','viscous':'粘性流动','boundary':'边界层','compress':'可压缩流','potential':'势流','oceanography':'物理海洋学'};
    ...
    const pct=Math.round(visited.size/ALL_MODS.length*100);
	    const serverAnswered=serverProgress?Number(serverProgress.answered||0):null;
	    const serverAccuracy=serverProgress&&serverAnswered?Number(serverProgress.accuracy||0):null;
	    return {
	      pct,
	      visited:visited.size,
	      total:ALL_MODS.length,
	      quizzes:serverProgress?serverAnswered:null,
	      avgScore:serverProgress?serverAccuracy:null,
	      localQuizDiagnostic:scs.length,
	      serverTruth:!!serverProgress
	    };
	  }catch(_){return {pct:0,visited:0,total:8,quizzes:null,avgScore:null,localQuizDiagnostic:0,serverTruth:false}}
	}
...
function renderGauge(){
  try{
    ...
    const mods=$('#gMods');if(mods)mods.textContent=p.visited;
	    const qz=$('#gQz');if(qz)qz.textContent=p.serverTruth?String(p.quizzes||0):'同步中';
  ...
  try{
    ...
    if(p.visited===0){
      ...
      cta='查看错题本';href='#tabsW';
	    }else if(p.serverTruth&&p.quizzes===0){
	      title='做一次测验，校准学习节奏';
      ...
      title='继续学《'+nm+'》';
	      desc='你已访问 '+p.visited+'/'+p.total+' 个模块；服务端累计正确率 '+(p.avgScore!=null?p.avgScore:'待同步')+'%。保持节奏，下一章见。';
...
window.addEventListener('fm:server-progress-snapshot',()=>{try{renderGauge();renderFocus()}catch(_){}});
