...
- `rerun-real-private-video-browser-gate` (release operator): command: `NODE_PATH=<codex primary runtime node_modules> node tools/check-private-video-management-browser.mjs --production --json`. Expected: 真实教师/学生账号 QA 证明存储链路，不输出私人登录材料。
