...
const version = round573.version;
...
const outJson = path.join(qaDir, 'round573-question-bank-home-visual-proof.json');
  ...
  return http.createServer((req, res) => {
    ...
    if (pathname === '/api/auth/me') {
      ...
      res.end(JSON.stringify({
        ok: true,
        authenticated: true,
        user: { username: 'round573-visual-student', role: 'student', access: 'active', status: 'active' }
      }));
      return;
    }
...
async function evaluatePage(page, viewport) {
  const screenshot = path.join(qaDir, `round573-question-bank-home-${viewport.name}.png`);
  ...
  return page.evaluate(({ version, viewport, screenshot }) => {
    ...
    const evidenceStrip = document.querySelector('[data-release-evidence-strip="round573-fluidity-live"]');
    const proofStrip = document.querySelector('[data-round572-proof-audit-strip="1"]');
    const flowRail = document.querySelector('[data-round573-fluidity-rail="1"]');
    const summaryLede = document.querySelector('.summary-lede');
    ...
    const summaryStyle = summaryLede ? getComputedStyle(summaryLede) : null;
    const flowStyle = flowRail ? getComputedStyle(flowRail) : null;
    return {
      viewport,
      screenshot,
      versionPresent: bodyText.includes(version),
      topLine,
      round573Visible: topLine.includes('Round573'),
      answerDepth145: /真题补答\s*145/.test(topLine),
      ...
      proofStripCounts: /422/.test(proofText) && /strict\s*0/i.test(proofText),
      evidenceStripVisible: Boolean(evidenceStrip?.offsetParent),
      flowRailVisible: Boolean(flowRail?.offsetParent),
      flowRailDisplay: flowStyle?.display || '',
      flowRailItems: document.querySelectorAll('[data-round573-flow]').length,
      summaryLedeVisible: Boolean(summaryLede?.offsetParent) && summaryStyle?.display !== 'none' && summaryStyle?.visibility !== 'hidden',
      noContentVisibilityAuto: offscreenSections.every((node) => getComputedStyle(node).contentVisibility !== 'auto'),
      noOldCurrentCounts: !/真题补答\s*(110|122|133)|重证\s*5|381\s*可参考|141\s*源文线索|381练习|141线索/.test(bodyText),
...
try {
  ...
  for (const viewport of viewports) {
    ...
    await page.addInitScript(() => {
      const now = Date.now();
      const user = { username: 'round573-visual-student', name: 'Round573 Visual QA', role: 'student' };
      const session = {
        version: 2,
        issuedAt: now,
...
const ok = results.every((result) => (
  ...
  && result.round573Visible
  && result.answerDepth145
  ...
  && result.flowRailVisible
  && result.flowRailItems === 4
  && result.summaryLedeVisible
