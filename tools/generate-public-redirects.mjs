...
<body>
  <main>
    <h1>正在进入主站</h1>
    <p>这个公开路径已迁移到 Cloudflare 源站。请点击按钮进入完整主站；进入前会先清理旧入口缓存。</p>
  ...
  <script>
    ...
    document.getElementById('targetLink').addEventListener('click', (event) => {
      event.preventDefault();
      Promise.race([clearOldPublicState(), timeout(600)]).finally(() => {
