...
import { round570 } from './round570-answer-depth-data.mjs';
...
const version = round570.version;
...
const outJson = path.join(qaDir, 'round570-question-bank-home-visual-proof.json');
  ...
  return http.createServer((req, res) => {
    ...
    if (pathname === '/api/auth/me') {
      ...
      res.end(JSON.stringify({
        ok: true,
        authenticated: true,
        user: { username: 'round570-visual-student', role: 'student', access: 'active', status: 'active' }
      }));
      return;
    }
...
async function evaluatePage(page, viewport) {
  const screenshot = path.join(qaDir, `round570-question-bank-home-${viewport.name}.png`);
  ...
  return page.evaluate(({ version, viewport, screenshot }) => {
    ...
    const evidenceStrip = document.querySelector('[data-release-evidence-strip="round570-auth-live"]');
    const proofStrip = document.querySelector('[data-round570-proof-audit-strip="1"]');
    ...
    return {
      ...
      topLine,
      round570Visible: topLine.includes('Round570'),
      answerDepth122: /真题补答\s*122/.test(topLine),
      ...
      noOldCurrentCounts: !/真题补答\s*110|重证\s*5|381\s*可参考|141\s*源文线索|381练习|141线索/.test(bodyText),
...
try {
  ...
  for (const viewport of viewports) {
    ...
    await page.addInitScript(() => {
      const now = Date.now();
      const user = { username: 'round570-visual-student', name: 'Round570 Visual QA', role: 'student' };
      const session = {
        version: 2,
        issuedAt: now,
...
const ok = results.every((result) => (
  ...
  && result.round570Visible
  && result.answerDepth122
