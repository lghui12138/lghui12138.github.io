...
const outJson = path.join(qaDir, 'round588-question-bank-home-conductor-visual-gate.json');

if (repoRoot.startsWith('/Volumes/mac_2T') || process.cwd().startsWith('/Volumes/mac_2T')) {
  throw new Error('Refusing to run Round588 visual QA from /Volumes/mac_2T during lifs isolation.');
}

fs.mkdirSync(qaDir, { recursive: true });

const qaUser = {
  username: 'round588-home-conductor-student',
  name: 'Round588 Home Conductor QA',
  role: 'student',
  access: 'active',
  status: 'active'
};
...
async function evaluateHomePage(browser, port, viewport) {
  ...
  await page.goto(localUrl(port, `/question-bank-home.html?edge_refresh=${encodeURIComponent(round588.version)}`), { waitUntil: 'domcontentloaded', timeout: 30000 });
  await page.waitForSelector('[data-round588-route-conductor="topline"]', { state: 'visible', timeout: 30000 });
  await page.waitForSelector('[data-round588-route-conductor="summary-actions"]', { state: 'visible', timeout: 30000 });
  await page.waitForTimeout(600);

  const screenshot = path.join(qaDir, `round588-question-bank-home-${viewport.name}.png`);
  const firstView = path.join(qaDir, `round588-question-bank-home-${viewport.name}-firstview.png`);
  ...
  const result = await page.evaluate(({ version, viewport, screenshot, firstView }) => {
    ...
    const summary = document.querySelector('[data-round588-route-conductor="summary"]');
    const topline = document.querySelector('[data-round588-route-conductor="topline"]');
    const entry = document.querySelector('[data-round588-route-conductor="primary-entry"]');
    const summaryRect = summary?.getBoundingClientRect();
    const toplineRect = topline?.getBoundingClientRect();
    const entryRect = entry?.getBoundingClientRect();
    const toplinePosition = topline ? getComputedStyle(topline).position : null;
    const summaryActionsInFirstViewport = Array.from(document.querySelectorAll('[data-round588-route-conductor="summary-actions"] a')).filter((node) => {
      if (!visible(node)) return false;
      const rect = node.getBoundingClientRect();
      return rect.top >= -1 && rect.bottom <= innerHeight - 8;
    }).length;
    return {
      viewport,
      screenshot,
      ...
      bodyMarker: document.body.getAttribute('data-round588-route-conductor-page') === '1',
      toplineVisible: visible(topline),
      summaryVisible: visible(summary),
      entryVisible: visible(entry),
      toplineInFirstViewport: Boolean(toplineRect && toplineRect.top < innerHeight),
      conductorTextVisible: ['Round588', '400/122', '422/8', '353/163', 'strict 0'].every((snippet) => firstViewportText.includes(snippet)),
      firstViewportUseful: ['400', '122', '422', 'strict'].every((snippet) => firstViewportText.includes(snippet)),
      ...
      majorGaps,
      routeInventory: {
        topActions: document.querySelectorAll('.workbench-topline__actions a').length,
      ...
      countsVisible: ['400 可参考', '122 条源文线索', '422 道', '163 深补'].every((snippet) => bodyText.includes(snippet)),
      ...
      noStaleVisibleCounts: !/(145 深补|153 深补|真题深度补答累计 145|381 练习|141 源文|132 条参考答案页|<b>Round584<\/b><span>入口首页密度升级<\/span>)/.test(bodyHtml),
      toplinePosition,
      stickyModeMatches: innerWidth <= 768 ? toplinePosition === 'static' : toplinePosition === 'sticky',
      summaryActionsInFirstViewport,
      horizontalOverflow: scrollWidth > innerWidth + 1,
      overflowNodes,
      touchFailures
    };
  }, { version: round588.version, viewport, screenshot, firstView });
...
async function main() {
  ...
  try {
    const viewports = [
      ...
      { name: 'tabletStatic768', width: 768, height: 1024 },
      { name: 'tabletSticky820', width: 820, height: 1180 },
      { name: 'tabletLandscape1024', width: 1024, height: 768 },
      { name: 'mobile390', width: 390, height: 844 },
      { name: 'mobileAndroid360', width: 360, height: 780 },
      { name: 'shortNarrow320', width: 320, height: 568 },
      { name: 'narrowReduced320', width: 320, height: 740, reducedMotion: true }
    ];
    const results = [];
    ...
    for (const result of results) {
      const gapTarget = result.viewport.width <= 390 ? 32 : 48;
      pushCheck(`Round588 markers visible ${result.viewport.name}`,
        ...
        && result.entryVisible,
        {
          ...
          bodyMarker: result.bodyMarker,
          toplineVisible: result.toplineVisible,
          summaryVisible: result.summaryVisible,
          entryVisible: result.entryVisible,
          summaryHeight: result.summaryHeight,
          entryTop: result.entryTop
        });
      pushCheck(`Round588 first viewport conductor ${result.viewport.name}`,
        result.toplineInFirstViewport
        && result.conductorTextVisible
        && result.firstViewportUseful
        && (result.viewport.width > 390 || result.summaryActionsInFirstViewport >= 4)
        && result.maxMajorGap <= gapTarget,
        {
          toplineInFirstViewport: result.toplineInFirstViewport,
          conductorTextVisible: result.conductorTextVisible,
          firstViewportUseful: result.firstViewportUseful,
          summaryActionsInFirstViewport: result.summaryActionsInFirstViewport,
          maxMajorGap: result.maxMajorGap,
          gapTarget,
          majorGaps: result.majorGaps
        });
      pushCheck(`Round588 route inventory preserved ${result.viewport.name}`,
        result.routeInventory.topActions === 4
      ...
      pushCheck(`Round588 counts and boundary visible ${result.viewport.name}`,
      ...
      pushCheck(`Round588 home no overflow and touch targets ${result.viewport.name}`,
        !result.horizontalOverflow
        && result.stickyModeMatches
        && result.overflowNodes.length === 0
        && result.touchFailures.length === 0,
        {
          horizontalOverflow: result.horizontalOverflow,
          toplinePosition: result.toplinePosition,
          stickyModeMatches: result.stickyModeMatches,
          overflowNodes: result.overflowNodes,
          touchFailures: result.touchFailures
        });
      pushCheck(`Round588 diagnostics clean ${result.viewport.name}`,
