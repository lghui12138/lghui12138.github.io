...
const outJson = path.join(qaDir, 'round579-real-exam-answer-render-visual-gate.json');
...
const qaUser = {
  username: 'round579-answer-render-student',
  name: 'Round579 Answer Render QA',
  role: 'student',
  access: 'active',
  status: 'active'
};
...
const samples = [
  { id: 'ocean-2018-04-02', year: '2018', snippet: '流量系数', screenshot: true },
  { id: 'ocean-2007-07-04', year: '2007', snippet: '粘性耗散', screenshot: true }
];
...
async function evaluateSample(browser, port, sample) {
  ...
  const href = `/modules/practice-dynamic.html?type=real&exam=${sample.year}-real-exam&year=${sample.year}&mode=normal&q=${encodeURIComponent(sample.id)}&edge_refresh=${encodeURIComponent(round579.version)}`;
  ...
  const screenshot = sample.screenshot ? path.join(qaDir, `round579-real-answer-render-${sample.id}.png`) : '';
  ...
  }, { id: sample.id, snippet: sample.snippet, version: round579.version, screenshot });
  await page.close();
