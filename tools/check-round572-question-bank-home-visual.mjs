...
import { round572 } from './round572-answer-depth-data.mjs';
...
const version = round572.version;
...
const outJson = path.join(qaDir, 'round572-question-bank-home-visual-proof.json');
  ...
  return http.createServer((req, res) => {
    ...
    if (pathname === '/api/auth/me') {
      ...
      res.end(JSON.stringify({
        ok: true,
        authenticated: true,
        user: { username: 'round572-visual-student', role: 'student', access: 'active', status: 'active' }
      }));
      return;
    }
...
async function evaluatePage(page, viewport) {
  const screenshot = path.join(qaDir, `round572-question-bank-home-${viewport.name}.png`);
  ...
  return page.evaluate(({ version, viewport, screenshot }) => {
    ...
    const evidenceStrip = document.querySelector('[data-release-evidence-strip="round572-auth-live"]');
    ...
    return {
      ...
      proofStripCounts: /422/.test(proofText) && /6/.test(proofText) && /strict\s*0/i.test(proofText),
...
try {
  ...
  for (const viewport of viewports) {
    ...
    await page.addInitScript(() => {
      const now = Date.now();
      const user = { username: 'round572-visual-student', name: 'Round572 Visual QA', role: 'student' };
      const session = {
        version: 2,
        issuedAt: now,
...
const ok = results.every((result) => (
  ...
  && result.round572Visible
  && result.answerDepth145
  && result.proofDepth422
  && result.proofRewrite6
  && result.reference400
  && result.source122
