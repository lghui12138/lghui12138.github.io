...
const outJson = path.join(qaDir, 'round571-answer-render-visual-gate.json');
...
const qaUser = {
  username: 'round571-answer-render-student',
  name: 'Round571 Answer Render QA',
  role: 'student',
  access: 'active',
  status: 'active'
};
...
const realExamBrowserSamples = [
  { id: 'ocean-2005-02-03', snippet: '不可压缩流动的运动学含义' },
  { id: 'ocean-2021-07-full', snippet: '开启瞬间速度为零' },
  { id: 'ocean-2000-07-03', snippet: '边界层连续方程和动量方程' },
  { id: 'ocean-2004-03', snippet: '一般 Cauchy 运动方程' }
];

const proofBrowserSnippets = new Map([
  ['181103-material-extracted-0482', '可压缩 Bernoulli'],
  ['181103-material-extracted-0030', '有向面积矢量'],
  ['181103-material-extracted-0246', 'Prandtl 边界层方程'],
  ['181103-material-extracted-0413', 'Einstein 求和约定'],
  ['181103-material-extracted-0083', 'Lamb 形式'],
  ['181103-material-extracted-0415', '对称部分']
]);
...
async function checkRealExamAnswer(browser, port, sample, index) {
  ...
  const href = `/modules/practice-dynamic.html?type=real&exam=${year}-real-exam&year=${year}&q=${encodeURIComponent(sample.id)}&edge_refresh=${encodeURIComponent(round571.version)}`;
  ...
  const screenshot = index === 0 ? path.join(qaDir, `round571-real-answer-render-${sample.id}.png`) : '';
  ...
  }, { id: sample.id, snippet: sample.snippet, version: round571.version, screenshot });
  await page.close();
...
async function check181103ProofAnswer(browser, port, item, index) {
  ...
  const screenshot = index === 0 ? path.join(qaDir, `round571-181103-answer-render-${item.id}.png`) : '';
  ...
  const result = await article.evaluate((node, { id, snippet, version, screenshot }) => {
    ...
    return {
      ...
      round571Summary: /Round571 proof-depth/.test(details?.querySelector('summary')?.innerText || ''),
  ...
  }, { id: item.id, snippet, version: round571.version, screenshot });
  await page.close();
  return { ...result, consoleMessages, pageErrors };
}
...
const realDataRows = realExamUpgrades.map((item) => {
  ...
  return {
    id: item.id,
    exists: Boolean(row),
    round571: row?.round571AnswerDepthUpgrade === true,
...
const ok = realDataRows.every((row) => row.exists && row.round571 && row.answerVisibleDataReady && row.trust === 'reference-answer-derived-verified' && row.strict === false)
  ...
  && proofBrowserRows.every((row) => row.articleVisible && row.answerOpen && row.answerBodyVisible && row.round571Summary && row.answerSnippetVisible && row.answerLength >= 650 && row.versionPresent && row.sourceLedger400 && !row.articleOverflow && row.pageErrors.length === 0);
