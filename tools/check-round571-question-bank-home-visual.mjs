...
import { round571 } from './round571-answer-depth-data.mjs';
...
const version = round571.version;
...
const outJson = path.join(qaDir, 'round571-question-bank-home-visual-proof.json');
  ...
  return http.createServer((req, res) => {
    ...
    if (pathname === '/api/auth/me') {
      ...
      res.end(JSON.stringify({
        ok: true,
        authenticated: true,
        user: { username: 'round571-visual-student', role: 'student', access: 'active', status: 'active' }
      }));
      return;
    }
...
async function evaluatePage(page, viewport) {
  const screenshot = path.join(qaDir, `round571-question-bank-home-${viewport.name}.png`);
  ...
  return page.evaluate(({ version, viewport, screenshot }) => {
    ...
    const evidenceStrip = document.querySelector('[data-release-evidence-strip="round571-auth-live"]');
    const proofStrip = document.querySelector('[data-round571-proof-audit-strip="1"]');
    ...
    return {
      ...
      topLine,
      round571Visible: topLine.includes('Round571'),
      answerDepth133: /真题补答\s*133/.test(topLine),
      ...
      noOldCurrentCounts: !/真题补答\s*(110|122)|重证\s*5|381\s*可参考|141\s*源文线索|381练习|141线索/.test(bodyText),
...
try {
  ...
  for (const viewport of viewports) {
    ...
    await page.addInitScript(() => {
      const now = Date.now();
      const user = { username: 'round571-visual-student', name: 'Round571 Visual QA', role: 'student' };
      const session = {
        version: 2,
        issuedAt: now,
...
const ok = results.every((result) => (
  ...
  && result.round571Visible
  && result.answerDepth133
