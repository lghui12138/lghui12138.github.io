...
if (repoRoot.startsWith('/Volumes/mac_2T') || process.cwd().startsWith('/Volumes/mac_2T')) {
  throw new Error('Refusing to run Round572 apply from /Volumes/mac_2T during lifs isolation.');
}
...
function mutateRealExamRows() {
  ...
  for (const upgrade of realExamUpgrades) {
    ...
    Object.assign(row, {
      ...
      round572AnswerDepthUpgrade: true,
      round572AnswerDepthUpgradeAt: now,
      round572AnswerDepthUpgradeVersion: round572.version,
      round572AnswerDepthDiagnosis: upgrade.diagnosis,
      round572AnswerDepthBoundaryNote: upgrade.boundaryNote,
      answerSource: {
        type: 'derived-reference',
        status: 'verified-derived-correct',
        strictAnswerPdfProof: false,
        boundary: 'Round572 derived reference answer; not strict original answer-PDF span/bbox/hash proof.',
...
function mutateAnswerChecks() {
  ...
  for (const upgrade of realExamUpgrades) {
    ...
    const existingKeys = Object.entries(checks.answerChecksById)
      ...
      .map(([key]) => key);
    const keys = existingKeys.length ? existingKeys : [`${upgrade.id}-round572-answer-depth`];
    ...
    for (const key of keys) {
      ...
      const row = {
        ...previous,
        id: previous.id || key,
        starterId: previous.starterId || `${upgrade.id}-round572`,
  ...
  checks.version = round572.version;
  checks.generatedAt = now;
  checks.source = 'round572-answer-depth-tenth-pass';
...
function mutate181103Rows() {
  ...
  for (const upgrade of proofDepthRewrites181103) {
    ...
    Object.assign(row, {
      ...
      round572ProofDepthRewrite: true,
      round572ProofDepthRewriteAt: now,
      round572ProofDepthRewriteVersion: round572.version,
      round572ProofDepthDiagnosis: upgrade.diagnosis,
      round572ProofDepthBoundaryNote: upgrade.boundaryNote
    });
    row.qualityFlags = Array.from(new Set([...(row.qualityFlags || []), 'round572-proof-depth-rewritten']));
    touchedRows.push({
      id: upgrade.id,
      title: row.title,
      type: row.type,
...
function replace181103ArticleAnswer(rel, id, answer) {
  ...
  const replacement = `<details open class="material-question-card__answer" data-181103-answer-html="1" data-round388-reference-answer-visible="1" data-round399-answer-default-visible="1" data-round399-answer-discoverable="1" aria-label="参考答案默认展开" data-round420-answer-trust-state="reference-answer-ready"><summary>参考答案（Round572 proof-depth 重证，默认展开，可收起）</summary><div data-round388-reference-answer-body="1">${answerToHtml(answer)}</div></details>`;
...
function updateQuestionBankIndex() {
  ...
  index.version = round572.version;
  index.currentEntryVersion = round572.version;
  index.updatedAt = now;
  ...
  if (bank) {
    ...
    bank.currentEntryVersion = round572.version;
...
function sync181103HtmlGlobalPages() {
  ...
  for (const rel of materialFiles) {
    ...
    text = replaceAll(text, round572.previousVersion, round572.version);
    text = text.replace(oldReleasePattern, round572.version);
  ...
  index = replaceAll(index, round572.previousVersion, round572.version);
  index = index.replace(/<body data-current-release="[^"]+" data-round394-181103-current-release="[^"]+">/, `<body data-current-release="${round572.version}" data-round394-181103-current-release="${round572.version}">`);
  index = index.replace(/<span>round555-181103-proof-depth-upgrade-20260628<\/span>/, `<span>${round572.version}</span>`);
  index = replaceAll(index, 'edge_refresh=round555-181103-proof-depth-upgrade-20260628', `edge_refresh=${round572.version}`);
  index = replaceAll(index, '522 核对 / 381 默认练习 / 381 可参考 / 0 待复核 / 141 线索', '522 核对 / 400 默认练习 / 400 可参考 / 0 待复核 / 122 线索');
  index = replaceAll(index, 'Round531-555 共 360 道证明/推导题深修', 'Round531-572 当前 422 道证明/推导题深修');
  index = replaceAll(index, 'Round531-555 已把 360 道', 'Round531-572 当前总账已到 422 道');
  ...
  index = replaceAll(index, '<span>Round555 第二十一批新增 10 道</span>', '<span>Round572 本轮重证 6 道</span>');
  index = replaceAll(index, '<span>后续可疑题继续排队</span>', '<span>后续薄证明继续排队</span><span>Round572 重证记录已发布</span>');
...
function updateLedgers(realRows, proofRows) {
  const realReport = {
    ...
    cumulativeAnswerDepthRows: round572.realExamCumulativeAnswerDepthRows,
    previousCumulativeAnswerDepthRows: round572.realExamPreviousAnswerDepthRows,
    strictAnswerPdfProofRows: round572.strictAnswerPdfProofRows,
    failedIds: [],
    rows: realRows
  };
  writeJson('data/fluid-round572-real-exam-answer-depth-upgrade.json', realReport);
  ...
  const proofReport = {
    ...
    cumulativeProofDepthRows: round572.proofDepthRows181103,
    cumulativePracticeRows: round572.referencePracticeRows181103,
    cumulativeSourceClueRows: round572.sourceClueOnlyRows181103,
    strictAnswerPdfProofRows: round572.strictAnswerPdfProofRows,
    failedIds: [],
    rows: proofRows
  };
  writeJson('data/fluid-round572-181103-proof-depth-rewrite.json', proofReport);
  ...
  const continuation = {
    ...
    summary: {
      rewrittenRows: proofDepthRewrites181103.length,
      rewrittenIds: proofDepthRewrites181103.map((item) => item.id),
      proofDepthRows: round572.proofDepthRows181103,
      practiceRows: round572.referencePracticeRows181103,
      sourceClueRows: round572.sourceClueOnlyRows181103,
      strictAnswerPdfProofRows: round572.strictAnswerPdfProofRows,
      boundary: 'Round572 re-deepens existing 181103 reference-answer-ready rows; strict answer-PDF proof remains separate at 0.'
    }
  };
  writeJson('data/fluid-round572-181103-proof-depth-continuation-ledger.json', continuation);
}
...
function updateSiteUpdates() {
  ...
  const top = {
    ...
    previousVersion: round572.previousVersion,
    date: '2026-06-29',
    title: 'Round572：第十轮 12 道真题过程补答，6 道 181103 证明重证，入口累计 145',
    summary: '本轮补强 12 道历年真题过程型参考答案，全部为新进入深补累计，使真题补答从 133 推到 145；并重证 6 道 181103 证明/推导题，把假设、方程、边界/符号、变形和结论检查写全。题库入口继续沿工作台样式压缩空白，同时同步 181103 的 400/122 当前口径。strictAnswerPdfProof 仍为 0，不把派生参考答案冒充为原卷答案 PDF 逐字证据。',
    links: [
      { label: '真题补答记录', href: '/data/fluid-round572-real-exam-answer-depth-upgrade.json' },
      { label: '181103 proof-depth 重证记录', href: '/data/fluid-round572-181103-proof-depth-rewrite.json' },
      { label: '题库入口', href: `/question-bank-home.html?edge_refresh=${round572.version}` }
    ],
    metrics: {
      realExamAnswerDepthRows: round572.realExamCumulativeAnswerDepthRows,
      realExamRoundUpgradeRows: round572.realExamRoundUpgradeRows,
      realExamNewUniqueRows: round572.realExamNewUniqueRows,
      proofDepthRows: round572.proofDepthRows181103,
      proofDepthRewriteRows: proofDepthRewrites181103.length,
      referencePracticeRows: round572.referencePracticeRows181103,
      sourceClueOnlyRows: round572.sourceClueOnlyRows181103,
      strictAnswerPdfProof: round572.strictAnswerPdfProofRows
    },
    boundary: {
  ...
  const nextUpdates = updates.filter((item) => item?.version !== round572.version);
...
function updateRoadmap() {
  ...
  roadmap.version = round572.version;
  roadmap.currentVersion = round572.version;
  roadmap.currentReleaseVersion = round572.version;
  roadmap.previousVersion = round572.previousVersion;
  roadmap.currentRound = 572;
  roadmap.lastIntegratedRound = 572;
  ...
  roadmap.latestRelease = {
    ...
    round: 572,
    previousVersion: round572.previousVersion,
    summary: 'Round572 upgrades 12 real-exam process answers, adds 12 new unique deep-answer rows for a cumulative 145, re-proves 6 existing 181103 proof-depth rows, and syncs current 181103 400/122 entry counts.'
  };
  roadmap.releaseGate ||= {};
  roadmap.releaseGate.currentVersion = round572.version;
  roadmap.releaseGate.expectedVersion = round572.version;
  roadmap.releaseGate.previousVersion = round572.previousVersion;
  roadmap.releaseGate.currentRound = 572;
  roadmap.releaseGate.lastUpdated = now;
  const round572Commands = [
    'node tools/apply-round572-answer-depth-tenth-pass.mjs',
    'node tools/check-round572-answer-depth-tenth-pass.mjs',
    'node tools/check-round572-question-bank-home-visual.mjs',
    'node tools/check-round572-answer-render-visual.mjs',
    'node tools/check-lghui-top-auth-chain.mjs --json --production --expected-version round572-answer-depth-tenth-pass-proof-ui-refresh-20260629',
    'node tools/check-authenticated-browser-gate.mjs --json --production --expected-version round572-answer-depth-tenth-pass-proof-ui-refresh-20260629'
  ];
  const previousCommands = (roadmap.releaseGate.commands || [])
    .filter((command) => !String(command).includes(round572.previousVersion))
    .filter((command) => !/answer-depth-eighth-pass|answer-depth-ninth-pass|round570-answer-depth|round571-answer-depth/.test(String(command)));
  roadmap.releaseGate.commands = Array.from(new Set([...round572Commands, ...previousCommands]));
  roadmap.round572AnswerDepthRows = realExamUpgrades.length;
  ...
  roadmap.round572ProofDepthRewriteRows = proofDepthRewrites181103.length;
  roadmap.proofDepthRows = round572.proofDepthRows181103;
  roadmap.strictAnswerPdfProof = round572.strictAnswerPdfProofRows;
...
function updateCurrentPages() {
  ...
  for (const rel of currentFiles) {
    let text = readText(rel);
    text = replaceAll(text, round572.previousVersion, round572.version);
    text = replaceAll(text, 'Round571', 'Round572');
    text = replaceAll(text, 'round571', 'round572');
    text = replaceAll(text, 'Round570', 'Round572');
    text = replaceAll(text, 'round570', 'round572');
    text = replaceAll(text, 'Round569', 'Round572');
    text = replaceAll(text, 'round569', 'round572');
    text = replaceAll(text, '真题补答 <strong>110</strong>', '真题补答 <strong>145</strong>');
    text = replaceAll(text, '真题补答 <strong>122</strong>', '真题补答 <strong>145</strong>');
    text = replaceAll(text, '真题补答 <strong>133</strong>', '真题补答 <strong>145</strong>');
    text = replaceAll(text, '真题补答 110', '真题补答 145');
    text = replaceAll(text, '真题补答 122', '真题补答 145');
    text = replaceAll(text, '真题补答 133', '真题补答 145');
    text = replaceAll(text, '110 深补', '145 深补');
    text = replaceAll(text, '122 深补', '145 深补');
    text = replaceAll(text, '133 深补', '145 深补');
    text = replaceAll(text, '110 道已做过程型补答', '145 道已做过程型补答');
    text = replaceAll(text, '122 道已做过程型补答', '145 道已做过程型补答');
    text = replaceAll(text, '133 道已做过程型补答', '145 道已做过程型补答');
    text = replaceAll(text, '<dt>真题补答</dt><dd>110 道</dd>', '<dt>真题补答</dt><dd>145 道</dd>');
    text = replaceAll(text, '<dt>真题补答</dt><dd>122 道</dd>', '<dt>真题补答</dt><dd>145 道</dd>');
    text = replaceAll(text, '<dt>真题补答</dt><dd>133 道</dd>', '<dt>真题补答</dt><dd>145 道</dd>');
    text = replaceAll(text, '累计 110 道历年真题', '累计 145 道历年真题');
    text = replaceAll(text, '累计 122 道历年真题', '累计 145 道历年真题');
    text = replaceAll(text, '累计 133 道历年真题', '累计 145 道历年真题');
    text = replaceAll(text, '本轮新增 6 道', '本轮新增 12 道');
    text = replaceAll(text, '新增证明深修 5 道，真题深度补答累计 110 道', '重证 6 道 181103 推导题，真题深度补答累计 145 道');
    text = replaceAll(text, '重证 6 道 181103 推导题，真题深度补答累计 122 道', '重证 6 道 181103 推导题，真题深度补答累计 145 道');
    text = replaceAll(text, '重证 6 道 181103 推导题，真题深度补答累计 133 道', '重证 6 道 181103 推导题，真题深度补答累计 145 道');
    ...
    text = replaceAll(text, '新增证明记录<span>Round572 / 重证 5；总 422</span>', '新增证明记录<span>Round572 / 重证 6；总 422</span>');
    text = replaceAll(text, '证明深修截至 Round572 共 422 道', '证明深修截至 Round572 共 422 道');
...
console.log(JSON.stringify({
  ...
  cumulative: {
    realExamAnswerDepthRows: round572.realExamCumulativeAnswerDepthRows,
    proofDepthRows181103: round572.proofDepthRows181103,
    referencePracticeRows181103: round572.referencePracticeRows181103,
    sourceClueOnlyRows181103: round572.sourceClueOnlyRows181103,
    strictAnswerPdfProofRows: round572.strictAnswerPdfProofRows
  }
}, null, 2));
