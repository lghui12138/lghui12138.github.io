...
console.log('清理所有2000-2024年期间的考试标题...\n');

// 定义需要删除的考试标题模式（更全面的匹配）
...
// 保存清理后的数据
const outputFile = './question-banks/真题_中国海洋大学_2000-2024_ultimate_cleaned_all.json';
...
const remainingExamTitles = cleanedQuestions.filter(q => {
    const title = q.title || '';
    return title.includes('考试') || 
           title.includes('招生') || 
           title.includes('试题') || 
           title.includes('注：') ||
           title.includes('科目') ||
           /\d{4}年.*(考试|招生|试题)/.test(title);
});
