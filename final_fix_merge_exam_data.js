...
function isValidIndependentQuestion(title) {
    ...
    // 排除明显的小问
    if (/^[0-9]+\s*\)/.test(title) || /^\([0-9]+\)/.test(title)) {
        return false;
    }
    ...
    Object.keys(yearGroups).forEach(year => {
        ...
        yearQuestions.forEach(q => {
            const tooSimilar = deduped.find(existing => {
                ...
                return similarity > 0.85;  // 进一步提高相似度阈值
            });
...
// 执行最终修复合并
console.log('开始最终修复合并...');
console.log(`原始数据量: ${rawData.length}`);

const mergedQuestions = finalFixMerge(rawData);
...
// 保存结果
fs.writeFileSync(
    './question-banks/真题_中国海洋大学_2000-2024_fixed.json',
    JSON.stringify(finalQuestions, null, 2),
    'utf8'
);

console.log('最终修复合并完成！数据已保存到: 真题_中国海洋大学_2000-2024_fixed.json');
...
console.log('\n修正年份示例:');
const yearCorrectionExamples = finalQuestions.filter(q => q.title.includes('2007年') && q.year !== 2007);
yearCorrectionExamples.slice(0, 3).forEach((item, index) => {
    console.log(`${index + 1}. [修正为${item.year}年] ${item.title.substring(0, 60)}...`);
});
